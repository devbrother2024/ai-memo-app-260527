...
- `@/utils/memoRepository` — Supabase CRUD 어댑터
- `@/utils/contentHash` — 요약 본문 해시
- `@/lib/supabase` — Supabase client
- `@/lib/auth` — 익명 세션 보장 helper
...
const {
  ...
  getMemoById,        // (id) => Memo | undefined (in-memory)
  ...
  syncMemoSummary,    // (id, summary, contentHash) => void — useSummary에서 호출
} = useMemos()
```
