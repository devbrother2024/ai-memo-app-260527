...
const SAMPLE_MEMOS = [
  ...
  {
    title: 'React 19 새로운 기능 학습',
    content:
      'React 19에서 새로 추가된 기능들을 학습해야 함:\n\n1. React Compiler\n2. Actions & useActionState\n3. useOptimistic Hook\n4. use() API\n5. Server Components 개선사항\n\n이번 주말에 공식 문서를 읽고 간단한 예제를 만들어보자.',
    category: 'study',
    tags: ['React', '학습', '개발'],
  },
  {
...
export async function seedSampleData(userId: string): Promise<void> {
  ...
  const rows = SAMPLE_MEMOS.map(m => ({ ...m, user_id: userId }))
  const { error } = await supabase.from('memos').insert(rows)
  if (error) {
    console.error('샘플 데이터 시딩 실패:', error.message, error.details, error.hint)
  }
}
