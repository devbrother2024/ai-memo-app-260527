...
메모 CRUD 기능을 제공하는 Next.js 웹 애플리케이션. Supabase(PostgreSQL)를 사용하여 데이터를 영속화하며, 익명 인증 + RLS로 사용자별 데이터를 분리한다. 카테고리 분류, 검색, AI 요약 기능을 지원한다.
...
- **Persistence:** Supabase (PostgreSQL) — 익명 인증 + RLS
...
2. Supabase 클라이언트는 `src/lib/supabase.ts` 단일 인스턴스만 사용
3. 메모 ID는 Supabase `gen_random_uuid()` DB 기본값을 사용 (클라이언트 uuid 생성 금지)
...
├── hooks/         # 커스텀 훅 (useMemos, useSummary)
├── lib/           # 외부 서비스 클라이언트 (supabase.ts, auth.ts)
├── types/         # TypeScript 타입 정의
└── utils/         # 유틸리티 함수 (memoRepository, contentHash)
```
